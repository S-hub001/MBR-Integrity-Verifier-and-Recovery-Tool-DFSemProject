...
def parse_gpt_header(header_bytes: bytes) -> dict: # dict will store the parsed fieldsfrom header
    ...
    partition_entries_lba = struct.unpack("<Q", header_bytes[72:80])[0] # bytes 72-79 for partition entries starting LBA, little-endian unsigned long long
    num_partition_entries = struct.unpack("<I", header_bytes[80:84])[0] # bytes 80-83 for number of partition entries, little-endian unsigned int
    size_of_partition_entry = struct.unpack("<I", header_bytes[84:88])[0] # bytes 84-87 for size of each partition entry, little-endian unsigned int
    partition_array_crc32 = struct.unpack("<I", header_bytes[88:92])[0]  # bytes 88-91 for partition array CRC32, little-endian unsigned int
...
def read_gpt_primary_table(image_path: str, header: dict) -> bytes:
    """
    Reads PRIMARY GPT partition table from disk image.
    """

    lba = header["partition_entries_lba"]
    num = header["num_partition_entries"]
    size = header["size_of_partition_entry"]
